package gewebe.sandbox.sunflowapiapi;
...
public class SunflowShader {
    ...
    public void applyCurrentShader(float r, float g, float b) {
        if (type == SHADER_AMBIENT_OCCLUSION) {
            setAmbientOcclusionShader(new Color(r, g, b), dark, samples, maxDist);
        } else if (type == SHADER_TEXTURED_AMBIENT_OCCLUSION) {
            setAmbientOcclusionShader(new Color(r, g, b), dark, samples, maxDist, texture);
        } else if (type == SHADER_CONSTANT) {
            setConstantShader(new Color(r, g, b));
        } else if (type == SHADER_DIFFUSE) {
            setDiffuseShader(new Color(r, g, b));
        } else if (type == SHADER_TEXTURED_DIFFUSE) {
            setDiffuseShader(new Color(r, g, b), texture);
        } else if (type == SHADER_GLASS) {
            setGlassShader(new Color(r, g, b), eta, absorptionDistance, absorptionColor);
        } else if (type == SHADER_MIRROR) {
            setMirrorShader(new Color(r, g, b));
        } else if (type == SHADER_PHONG) {
            setPhongShader(new Color(r, g, b), specular, power, samples);
        } else if (type == SHADER_TEXTURED_PHONG) {
            setPhongShader(new Color(r, g, b), specular, power, samples, texture);
        } else if (type == SHADER_SHINY_DIFFUSE) {
            setShinyDiffuseShader(new Color(r, g, b), shiny);
        } else if (type == SHADER_TEXTURED_SHINY_DIFFUSE) {
            setShinyDiffuseShader(new Color(r, g, b), shiny, texture);
        } else if (type == SHADER_UBER) {
            setUberShader(new Color(r, g, b), specular, diffuseTexture, specularTexture, diffuseBlend, specularBlend, glossyness, samples);
        } else if (type == SHADER_WARD) {
            setWardShader(new Color(r, g, b), specular, roughnessX, roughnessY, samples);
        } else if (type == SHADER_TEXTURED_WARD) {
            setWardShader(new Color(r, g, b), specular, roughnessX, roughnessY, samples, texture);
        } else if (type == SHADER_WIREFRAME) {
            setWireframeShader(new Color(r, g, b), fillColor, width);
        }
    }
...
//		set parameter
        sunflow.parameter("bright",
                          colorSpace,
                          bright.getRed() / (float) 255,
                          bright.getGreen() / (float) 255,
                          bright.getBlue() / (float) 255);
...
//		set parameter
        sunflow.parameter("diffuse",
                          colorSpace,
                          color.getRed() / (float) 255,
                          color.getGreen() / (float) 255,
...
//		set parameter
        ...
        sunflow.parameter("absorption.color",
                          colorSpace,
                          absorptionColor.getRed() / (float) 255,
                          absorptionColor.getGreen() / (float) 255,
                          absorptionColor.getBlue() / (float) 255);
...
//		set parameter
        sunflow.parameter("diffuse",
                          colorSpace,
                          diffuse.getRed() / (float) 255,
                          diffuse.getGreen() / (float) 255,
                          diffuse.getBlue() / (float) 255);
        sunflow.parameter("specular",
                          colorSpace,
                          specular.getRed() / (float) 255,
                          specular.getGreen() / (float) 255,
                          specular.getBlue() / (float) 255);
...
//		set shader
     ...
     */
    public void setUberShader(Color diffuse,
                              Color specular,
...
//		set parameter
        sunflow.parameter("line",
                          colorSpace,
                          lineColor.getRed() / (float) 255,
                          lineColor.getGreen() / (float) 255,
                          lineColor.getBlue() / (float) 255);
        sunflow.parameter("fill",
                          colorSpace,
                          fillColor.getRed() / (float) 255,
                          fillColor.getGreen() / (float) 255,
                          fillColor.getBlue() / (float) 255);
