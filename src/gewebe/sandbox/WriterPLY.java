...
public class WriterPLY extends PGraphics {
    ...
    public void setPath(String path) {
        ...
        if (file == null) {
            throw new RuntimeException(WriterPLY.class.getSimpleName()
    ...
    private static PVector findPerpVec(PVector v) {
        ...
        if (mDot < 1.0f && mDot > -1.0f) {
            ...
            System.out.println("### " + WriterPLY.class.getSimpleName() + " / problem finding perp vector. ");
        }
    ...
    public void beginShape(int kind) {
        ...
        if ((shape != LINES)
            ...
            && (shape != POLYGON)) {
            String err
                    = WriterPLY.class.getSimpleName() + "can only be used with beginRaw(), "
                      + "because it only supports lines and triangles";
            throw new RuntimeException(err);
        }

        if ((shape == POLYGON) && fill) {
            throw new RuntimeException(WriterPLY.class.getSimpleName() + "only supports non-filled shapes.");
        }
